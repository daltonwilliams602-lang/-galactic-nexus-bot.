...
# Refresh only the exact setup templates authored by this bot.
LIVE_CONTENT = {
    'incoming-transmissions': CONTENT['incoming-transmissions'].split('\n\n**PRIVATE FOUNDER TESTING:')[0] + '\n\n**THE NEXUS IS OPEN.** Use `/join` to get started.',
    'announcements': '**THE GALACTIC NEXUS IS OPEN**\nRead #rules, use `/join`, then choose Jedi or Sith with `/path`. Earn XP through eligible chat, voice participation, and events. Ranks through Knight/Warrior advance automatically; Master/Lord require two different staff approvals.',
